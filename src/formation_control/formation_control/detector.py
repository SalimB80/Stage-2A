...
# Cyan HSV thresholds - TO BE CALIBRATED with tools/calibrate_hsv.py
...
def detect_helmet(bgr_image, min_area_frac=0.0015):
    """Return (bearing_norm, area_ratio, rectangular) or None.
    bearing_norm in [-1,1] : -1 = helmet on the left of the image."""
...
def bearing_to_angle(bearing_norm):
    """Real angle (rad) in the robot frame. + = to the left."""
    # bearing_norm +1 = right of image = -robot angle (REP-103: +y is left)
    return -bearing_norm * (CAMERA_HFOV / 2.0)


def range_from_lidar(scan, angle, window=3, max_valid=8.0):
    """Lidar distance (m) in the direction 'angle' (rad), or None.
    Handles 0..2pi lidars: the angle is normalised before indexing."""
    n = len(scan.ranges)
    if n == 0:
        return None
    # Normalise the angle into [angle_min, angle_min + 2pi[
    span = scan.angle_max - scan.angle_min
    a = (angle - scan.angle_min) % (2 * math.pi)
    idx = int(round(a / scan.angle_increment))
    # Circular window around the index
    valid = []
